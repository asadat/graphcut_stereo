...
void update_event(int ms)
{
   // static FeatureTracker featureTracker(makeVector(0,0,3));
    ...
    Stereo::Instance()->hanldeKeyPressed(Key, doUpdate);
    // actions:
...
//        }
    ...
    gstereo = new GraphcutStereo(argc, argv);
...
void Stereo::hanldeKeyPressed(std::map<unsigned char, bool> &key, bool &updateKey)
{
//    updateKey = true;
...
int main(int argc, char **argv)
{
   ...
   // printf("\t2 ....... Toggle Sensing\n\n");
    Stereo::Instance(argc, argv);
    Stereo::Instance()->mainLoop();
